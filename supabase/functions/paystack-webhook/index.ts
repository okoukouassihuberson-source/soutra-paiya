...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (type === "charge.success" && reference) {
      ...
      // Phase 14 : si c'est un paiement d'abonnement (sp-sub-), activer l'abo
      // après le settle. activate_subscription est idempotent.
      if (reference.startsWith("sp-sub-")) {
        ...
        const durationDays = Number(
          (tx as { metadata?: { duration_days?: number } } | null)?.metadata?.duration_days ?? 30,
        );
        const { data: subResult, error: subErr } = await svc.rpc("activate_subscription", {
          p_reference: reference,
          p_duration_days: durationDays,
        });
        if (subErr) console.error("[webhook] activate_subscription:", subErr);
        else console.log(`[webhook] subscription ${reference} -> ${subResult}`);
      }
    } else if (
